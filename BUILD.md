...
- **Importante**: También instala las dependencias del servidor: `cd server && npm install` (o `npm install`)
...
### Si aparece el error "Cannot create symbolic link":

Este error ocurre en Windows porque `electron-builder` necesita crear enlaces simbólicos.
- **Solución 1**: Ejecuta la terminal (PowerShell/CMD) como **Administrador**.
- **Solución 2**: Activa el **"Modo de programador"** en la configuración de Windows (recomendado).
...
1. **Verifica los logs**: Ejecuta el instalador desde la terminal para ver errores:
   ```powershell
   .\release\WhatyBot-DxS Setup 1.0.4.exe
   ```
